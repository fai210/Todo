...
export default function TodoItem({todo, onCompletedChange, onDelete}:TodoItemProps) {
  return (
    <div className=" flex items-center gap-1 ">
        <label className="flex itmes-center gap-2 border rounded-md p-2 border-gray-400 bg-white hover:bg-slate-50 grow">
        ...
        <button  onClick={() => onDelete(todo.id) } className="p-2">
            <Trash2 size={20} className="text-gray-500 placeholder:"></Trash2>
        </button>
    </div>
  )
}
